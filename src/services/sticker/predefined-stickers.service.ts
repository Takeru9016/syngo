...
class PredefinedStickersService {
  ...
  getAll(): Sticker[] {
    ...
    const stickers: Sticker[] = this.manifest.stickers.map((item) => {
      ...
      // Build full Cloudinary URL for push notifications
      // Uses expo-constants to get the cloud name from app config
      const cloudName =
        require("expo-constants").default?.expoConfig?.extra
          ?.cloudinaryCloudName;
      const remoteUrl =
        item.remoteUrl && cloudName ?
          `https://res.cloudinary.com/${cloudName}/image/upload/${item.remoteUrl}`
        : undefined;
      ...
      return {
        ...
        remoteUrl, // Full Cloudinary URL for push notifications
        createdBy: "system",
        createdAt: 0,
        isPredefined: true,
