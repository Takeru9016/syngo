...
export class NudgeService {
     ...
     */
    static async sendNudge(): Promise<void> {
     ...
     */
    static async getNudgeHistory(pairId: string, limitCount = 50): Promise<Nudge[]> {
        try {
            const nudgesRef = collection(db, `pairs/${pairId}/nudges`);
            const q = query(nudgesRef, orderBy("createdAt", "desc"), limit(limitCount));
